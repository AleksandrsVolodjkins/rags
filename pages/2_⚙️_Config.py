...
if current_state.agent_builder is not None:
    ...
    with st.expander("Loaded Data (Expand to view)"):
        ...
        # addad 240326
        if directory_path:  # Check if the path is not empty
            ...
            try:
                ...
                if files:
                    ...
                    for file in files:
                        st.write(file)
                        # Here you can add more processing for each file
                else:
                    st.write("The directory is empty.")
            except FileNotFoundError:
                st.error("The directory was not found. Please enter a valid path.")
        #added 
